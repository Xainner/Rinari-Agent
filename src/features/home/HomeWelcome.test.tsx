...
it('preserves the same composer DOM and focus when entering the conversation', async () => {
  ...
  composer.focus()
  ;(composer as HTMLTextAreaElement).setSelectionRange(2, 5)
  ;(composer as HTMLTextAreaElement).style.height = '96px'
  rerender(content(true))
  expect(screen.getByRole('textbox', {name:'Composer persistente'})).toBe(composer)
  expect(document.activeElement).toBe(composer)
  expect((composer as HTMLTextAreaElement).selectionStart).toBe(2)
  expect((composer as HTMLTextAreaElement).selectionEnd).toBe(5)
