// @generated by scripts/generate-protocol.mjs — DO NOT EDIT.
// Source: Rinari Engine Protocol v1 (sha256:916b4e0e7492f5ed2c2a2e65ab0971da8f458c380055967652128ff9d6251ca9)

use serde::{Deserialize, Serialize};

macro_rules! define_methods {
    ($( $variant:ident => $name:literal ),* $(,)?) => {
        #[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
        pub enum Method { $( $variant ),* }
        impl Method {
            pub fn as_str(self) -> &'static str {
                match self { $( Method::$variant => $name ),* }
            }
            pub const ALL: &'static [Method] = &[ $( Method::$variant ),* ];
        }
    };
}

define_methods! {
    EngineInfo => "engine.info",
    RuntimeSnapshotGet => "runtime.snapshot.get",
    SessionMove => "session.move",
    WorkspaceFileRead => "workspace.file.read",
    WorkspacePreviewStart => "workspace.preview.start",
    WorkspacePreviewStatus => "workspace.preview.status",
    WorkspacePreviewStop => "workspace.preview.stop",
    QuestionList => "question.list",
    QuestionResolve => "question.resolve",
    SessionList => "session.list",
    SessionGet => "session.get",
    SessionCreate => "session.create",
    SessionOpen => "session.open",
    SessionRename => "session.rename",
    SessionArchive => "session.archive",
    SessionRestore => "session.restore",
    SessionDelete => "session.delete",
    SessionFork => "session.fork",
    SessionHistory => "session.history",
    SessionModeSet => "session.mode.set",
    SessionModelSet => "session.model.set",
    SessionPermissionGet => "session.permission.get",
    SessionPermissionSet => "session.permission.set",
    SessionTurnStart => "session.turn.start",
    TargetList => "target.list",
    TargetAdd => "target.add",
    OperationStart => "operation.start",
    OperationGet => "operation.get",
    OperationCancel => "operation.cancel",
    SessionTurnCancel => "session.turn.cancel",
    SessionEvents => "session.events",
    SessionTimeline => "session.timeline",
    ApprovalResolve => "approval.resolve",
    TurnChangesGet => "turn.changes.get",
    TurnChangesReview => "turn.changes.review",
    TurnChangesUndoPreview => "turn.changes.undo.preview",
    TurnChangesUndo => "turn.changes.undo",
    ProjectList => "project.list",
    ProjectGet => "project.get",
    ProjectAdd => "project.add",
    ProjectUpdate => "project.update",
    ProjectRemove => "project.remove",
    ProjectListRecent => "project.list_recent",
    ProjectOpen => "project.open",
    ProjectStatus => "project.status",
    ProjectIntelligence => "project.intelligence",
    ProjectTrust => "project.trust",
    ProjectChanges => "project.changes",
    ProjectDiff => "project.diff",
    ProviderList => "provider.list",
    ProviderGet => "provider.get",
    ProviderCreate => "provider.create",
    ProviderUpdate => "provider.update",
    ProviderRemove => "provider.remove",
    ProviderTest => "provider.test",
    ProviderDiscover => "provider.discover",
    ProviderUse => "provider.use",
    ModelList => "model.list",
    ModelGet => "model.get",
    ModelAdd => "model.add",
    ModelAlias => "model.alias",
    ModelRemove => "model.remove",
    ModelUse => "model.use",
    ModelDiscover => "model.discover",
    ModelDiscoveryStart => "model.discovery.start",
    ModelRefresh => "model.refresh",
    ModelTest => "model.test",
    ModelCapabilities => "model.capabilities",
    AgentList => "agent.list",
    AgentConfigGet => "agent.config.get",
    AgentConfigSet => "agent.config.set",
    SoulList => "soul.list",
    SoulGet => "soul.get",
    SoulCreate => "soul.create",
    SoulUpdate => "soul.update",
    SoulRemove => "soul.remove",
    SoulActivate => "soul.activate",
    SoulGetEffective => "soul.get_effective",
    SessionSoulSet => "session.soul.set",
    SessionSoulClear => "session.soul.clear",
    McpList => "mcp.list",
    McpGet => "mcp.get",
    McpCreate => "mcp.create",
    McpRemove => "mcp.remove",
    McpEnable => "mcp.enable",
    McpDisable => "mcp.disable",
    McpTest => "mcp.test",
    PluginList => "plugin.list",
    PluginGet => "plugin.get",
    PluginEnable => "plugin.enable",
    PluginDisable => "plugin.disable",
    PluginDiagnostics => "plugin.diagnostics",
    ToolList => "tool.list",
    PolicyGet => "policy.get",
    TaskTree => "task.tree",
    TaskGet => "task.get",
    VerificationLatest => "verification.latest",
    VerificationPlan => "verification.plan",
    CheckpointList => "checkpoint.list",
    CheckpointShow => "checkpoint.show",
    CheckpointRestore => "checkpoint.restore",
    PtyStart => "pty.start",
    PtyWrite => "pty.write",
    PtyResize => "pty.resize",
    PtyRead => "pty.read",
    PtyList => "pty.list",
    PtyTerminate => "pty.terminate",
    ProfileBundleList => "profile_bundle.list",
    ProfileBundleGet => "profile_bundle.get",
    ProfileBundleCreate => "profile_bundle.create",
    ProfileBundleRemove => "profile_bundle.remove",
    ProfileBundleApply => "profile_bundle.apply",
    SessionQueueAdd => "session.queue.add",
    SessionQueueList => "session.queue.list",
    SessionQueueClear => "session.queue.clear",
    SessionQueueResume => "session.queue.resume",
    SessionPeerGroupSet => "session.peer_group.set",
    SessionPeerGroupGet => "session.peer_group.get",
    SessionPeerGroupRevoke => "session.peer_group.revoke",
    SessionPeerMessageList => "session.peer_message.list",
    SessionPeerMessageCancel => "session.peer_message.cancel",
    SessionPeerMessageForward => "session.peer_message.forward",
    SessionBranch => "session.branch",
    WorkspaceFileSearch => "workspace.file.search",
    ArtifactList => "artifact.list",
    ArtifactRead => "artifact.read",
    ArtifactExport => "artifact.export",
    ContextGet => "context.get",
    MemoryList => "memory.list",
    MemorySearch => "memory.search",
    MemoryGet => "memory.get",
    MemoryRemember => "memory.remember",
    MemoryUpdate => "memory.update",
    MemoryForget => "memory.forget",
    MemoryCandidatesList => "memory.candidates.list",
    MemoryCandidateResolve => "memory.candidate.resolve",
    ConversationMemoryStatus => "conversation.memory.status",
    ConversationMemoryExclude => "conversation.memory.exclude",
    ConversationDelete => "conversation.delete",
    MemoryLedgerExport => "memory.ledger.export",
    MemoryLedgerImport => "memory.ledger.import",
    UsageGet => "usage.get",
    SessionClose => "session.close",
    ChannelPending => "channel.pending",
    ChannelResolve => "channel.resolve",
    ArtifactReceiveImage => "artifact.receive_image",
    ArtifactDeleteMedia => "artifact.delete_media",
    SessionImageSupport => "session.image_support",
    ArtifactMediaList => "artifact.media_list",
    AttachmentPrepare => "attachment.prepare",
    AttachmentPreview => "attachment.preview",
    AttachmentPrepareStart => "attachment.prepare.start",
    AttachmentPrepareGet => "attachment.prepare.get",
    AttachmentPrepareCancel => "attachment.prepare.cancel",
    ModelVisionSet => "model.vision.set",
    BrowserViewGet => "browser.view.get",
    BrowserContextGet => "browser.context.get",
    BrowserContextPrepare => "browser.context.prepare",
    BrowserControlSet => "browser.control.set",
    HostBrowserRegister => "host.browser.register",
    HostBrowserUnregister => "host.browser.unregister",
    HostBrowserReply => "host.browser.reply",
    HostBrowserEvent => "host.browser.event",
    WorkspaceProcessList => "workspace.process.list",
    WorkspaceProcessRead => "workspace.process.read",
    WorkspaceProcessStop => "workspace.process.stop",
    VisionSettingsGet => "vision.settings.get",
    VisionSettingsSet => "vision.settings.set",
    ContextSettingsGet => "context.settings.get",
    ContextSettingsSet => "context.settings.set",
    ContextCompact => "context.compact",
    ContextStatus => "context.status",
}

pub const ENGINE_EVENTS: &[&str] = &[
    "turn.started",
    "turn.completed",
    "turn.failed",
    "turn.cancelled",
    "host.browser.request",
    "browser.control.changed",
    "turn.stopped",
    "model.started",
    "model.content.delta",
    "model.content.completed",
    "model.completed",
    "model.failed",
    "tool.requested",
    "tool.started",
    "tool.output.delta",
    "tool.completed",
    "tool.failed",
    "tool.cancelled",
    "approval.requested",
    "approval.resolved",
    "approval.expired",
    "question.requested",
    "question.resolved",
    "question.expired",
    "session.moved",
    "governor.progress",
    "governor.nudge",
    "governor.compact",
    "governor.consolidate",
    "governor.stop",
    "usage.updated",
    "agent.started",
    "agent.completed",
    "agent.failed",
    "verification.started",
    "verification.completed",
    "verification.failed",
    "model.discovery.completed",
    "model.discovery.failed",
    "turn.changes.completed",
    "turn.changes.undo.started",
    "turn.changes.undo.completed",
    "turn.changes.undo.conflict",
    "workspace.changed",
    "channel.requested",
    "agent.activity",
    "vision.started",
    "vision.completed",
    "vision.failed",
    "vision.cancelled",
    "session.peer.group.updated",
    "session.peer.message",
    "session.peer.message.updated",
];

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct Attachment {
    pub id: String,
    pub path: String,
    pub name: String,
    #[serde(default)]
    pub mime_type: Option<String>,
    #[serde(default)]
    pub size: Option<i64>,
    pub source: String,
    #[serde(default)]
    pub data_url: Option<String>,
    #[serde(default)]
    pub uri: Option<String>,
    #[serde(default)]
    pub sha256: Option<String>,
    #[serde(default)]
    pub kind: Option<String>,
    #[serde(default)]
    pub derived_uri: Option<String>,
    #[serde(default)]
    pub status: Option<String>,
    #[serde(default)]
    pub ocr: Option<bool>,
    #[serde(default)]
    pub truncated: Option<bool>,
    #[serde(default)]
    pub error: Option<String>,
    #[serde(default)]
    pub page_range: Option<String>,
    #[serde(default)]
    pub visual_pages: Option<Vec<i64>>,
    #[serde(default)]
    pub images: Option<Vec<serde_json::Value>>,
    #[serde(default)]
    pub warning: Option<String>,
    #[serde(default)]
    pub content_type: Option<String>,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct SessionSummary {
    pub id: String,
    pub kind: String,
    #[serde(default)]
    pub title: Option<String>,
    pub mode: String,
    pub state: String,
    pub updated_at: String,
    #[serde(default)]
    pub project_id: Option<String>,
    #[serde(default)]
    pub project_root: Option<String>,
    #[serde(default)]
    pub current_cwd: Option<String>,
    #[serde(default)]
    pub git_branch: Option<String>,
    pub last_active_at: String,
    pub provider_id: String,
    pub model_id: String,
    pub permission_profile: String,
    pub effective_permission_profile: String,
    #[serde(default)]
    pub read_permission_profile: Option<String>,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ProjectSummary {
    pub id: String,
    pub root: String,
    pub canonical_root: String,
    pub name: String,
    pub description: String,
    pub pinned: bool,
    pub archived: bool,
    #[serde(default)]
    pub git_fingerprint: Option<String>,
    pub created_at: String,
    pub updated_at: String,
    pub last_opened_at: String,
    #[serde(default)]
    pub active_session_id: Option<String>,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct UsageSnapshot {
    pub model_calls: i64,
    pub tool_calls: i64,
    pub network_calls: i64,
    pub subagent_calls: i64,
    pub max_recursion_depth: i64,
    pub input_tokens: i64,
    pub output_tokens: i64,
    pub wall_time_s: f64,
    #[serde(default)]
    pub estimated_cost: Option<f64>,
    pub exhausted: Vec<String>,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct GovernorSnapshot {
    pub execution: String,
    pub recovery_attempts: i64,
    pub max_recovery_attempts: i64,
    pub compactions: i64,
    pub last_compacted_history_size: i64,
    #[serde(default)]
    pub context_pressure: Option<f64>,
    #[serde(default)]
    pub progress: Option<serde_json::Value>,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ProtocolError {
    pub code: String,
    pub message: String,
    pub retryable: bool,
    pub details: serde_json::Value,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct QuestionOption {
    pub label: String,
    #[serde(default)]
    pub description: Option<String>,
    #[serde(default)]
    pub recommended: Option<bool>,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct UserQuestion {
    pub id: String,
    pub title: String,
    #[serde(default)]
    pub options: Option<Vec<QuestionOption>>,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct QuestionRequest {
    pub request_id: String,
    pub session_id: String,
    pub turn_id: String,
    pub status: String,
    pub questions: Vec<UserQuestion>,
    #[serde(default)]
    pub answers: Option<serde_json::Value>,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct FilePreview {
    pub path: String,
    pub name: String,
    pub content: String,
    pub language: String,
    pub size: i64,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct WebPreview {
    pub preview_id: String,
    pub session_id: String,
    pub url: String,
    pub kind: String,
    pub ready: bool,
    pub revision: i64,
    #[serde(default)]
    pub command: Option<String>,
    #[serde(default)]
    pub error: Option<String>,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ToolAvailability {
    #[serde(default)]
    pub available: Option<bool>,
    pub reason: String,
    pub requires: Vec<String>,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ToolSummary {
    pub name: String,
    pub description: String,
    pub risk: String,
    pub side_effects: String,
    pub namespace: String,
    #[serde(default)]
    pub capabilities: Option<Vec<String>>,
    #[serde(default)]
    pub permissions: Option<Vec<String>>,
    pub always_loaded: bool,
    #[serde(default)]
    pub availability: Option<ToolAvailability>,
    #[serde(default)]
    pub input_schema: Option<serde_json::Value>,
    #[serde(default)]
    pub output_schema: Option<serde_json::Value>,
    #[serde(default)]
    pub concurrency: Option<String>,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct BrowserPage {
    pub target_id: String,
    pub title: String,
    pub url: String,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct BrowserView {
    pub state: String,
    pub session_id: String,
    #[serde(default)]
    pub instance: Option<String>,
    #[serde(default)]
    pub image: Option<String>,
    #[serde(default)]
    pub url: Option<String>,
    #[serde(default)]
    pub target_id: Option<String>,
    #[serde(default)]
    pub error: Option<String>,
    #[serde(default)]
    pub pages: Option<Vec<BrowserPage>>,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ManagedProcess {
    pub id: String,
    pub kind: String,
    pub command: String,
    pub cwd: String,
    #[serde(default)]
    pub url: Option<String>,
    pub running: bool,
    pub can_stop: bool,
    #[serde(default)]
    pub pid: Option<i64>,
    #[serde(default)]
    pub started_at: Option<f64>,
    #[serde(default)]
    pub exit_code: Option<i64>,
    #[serde(default)]
    pub generation: Option<i64>,
    #[serde(default)]
    pub ended_at: Option<f64>,
    #[serde(default)]
    pub exit_reason: Option<String>,
    #[serde(default)]
    pub readiness: Option<String>,
    #[serde(default)]
    pub readiness_checked_at: Option<f64>,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ProcessOutput {
    pub process: ManagedProcess,
    pub stdout: String,
    pub stderr: String,
    pub truncated: bool,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ViewedImage {
    pub uri: String,
    pub name: String,
    pub path: String,
    pub width: i64,
    pub height: i64,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct AttachmentPreviewOptions {
    pub uri: String,
    #[serde(default)]
    pub max_bytes: Option<i64>,
    #[serde(default)]
    pub max_dimension: Option<i64>,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct VisionSettings {
    pub mode: String,
    #[serde(default)]
    pub model_id: Option<String>,
    pub confirm_unknown: bool,
    #[serde(default)]
    pub model_overrides: Option<serde_json::Value>,
    #[serde(default)]
    pub execution: Option<ModelExecutionSettings>,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct VisionActivity {
    pub vision_id: String,
    pub route: String,
    pub provider_id: String,
    pub model_id: String,
    pub question: String,
    pub images: Vec<ViewedImage>,
    #[serde(default)]
    pub analysis: Option<String>,
    #[serde(default)]
    pub artifact_uri: Option<String>,
    #[serde(default)]
    pub cached: Option<bool>,
    #[serde(default)]
    pub error: Option<String>,
    #[serde(default)]
    pub model_name: Option<String>,
    #[serde(default)]
    pub provider_name: Option<String>,
    #[serde(default)]
    pub attempt_id: Option<i64>,
    #[serde(default)]
    pub error_code: Option<String>,
    #[serde(default)]
    pub cache_key: Option<String>,
    #[serde(default)]
    pub origin: Option<String>,
    #[serde(default)]
    pub message_ref: Option<String>,
    #[serde(default)]
    pub tool_call_id: Option<String>,
    #[serde(default)]
    pub partial: Option<bool>,
    #[serde(default)]
    pub stop_reason: Option<String>,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct VisualRouteStatus {
    pub available: bool,
    pub route: String,
    pub reason: String,
    pub model_id: String,
    #[serde(default)]
    pub vision: Option<bool>,
    #[serde(default)]
    pub destination_model_id: Option<String>,
    #[serde(default)]
    pub destination_model: Option<String>,
    #[serde(default)]
    pub destination_provider: Option<String>,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ModelExecutionSettings {
    pub max_concurrency: i64,
    pub providers: serde_json::Value,
    pub models: serde_json::Value,
    #[serde(default)]
    pub timeouts: Option<serde_json::Value>,
    #[serde(default)]
    pub provider_timeouts: Option<serde_json::Value>,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ContextSettings {
    pub enabled: bool,
    pub compact_at_percent: i64,
    #[serde(default)]
    pub model_id: Option<String>,
    pub model_windows: serde_json::Value,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ContextStatus {
    pub window_tokens: i64,
    pub window_source: String,
    pub window_estimated: bool,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ContextActivity {
    pub compaction_id: String,
    #[serde(default)]
    pub session_id: Option<String>,
    #[serde(default)]
    pub turn_id: Option<String>,
    pub reason: String,
    pub status: String,
    #[serde(default)]
    pub window_tokens: Option<i64>,
    #[serde(default)]
    pub window_source: Option<String>,
    #[serde(default)]
    pub used_tokens: Option<i64>,
    #[serde(default)]
    pub after_tokens: Option<i64>,
    #[serde(default)]
    pub usage_source: Option<String>,
    #[serde(default)]
    pub error: Option<String>,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ToolResultDelivery {
    pub result_id: String,
    pub observation: String,
    #[serde(default)]
    pub source_partial: Option<bool>,
    #[serde(default)]
    pub delivery_partial: Option<bool>,
    #[serde(default)]
    pub result_ref: Option<String>,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct MessageOrigin {
    pub kind: String,
    #[serde(default)]
    pub message_id: Option<String>,
    #[serde(default)]
    pub source_session_id: Option<String>,
    #[serde(default)]
    pub source_turn_id: Option<String>,
    #[serde(default)]
    pub source_label: Option<String>,
    #[serde(default)]
    pub group_id: Option<String>,
    #[serde(default)]
    pub chain_id: Option<String>,
    #[serde(default)]
    pub hop: Option<i64>,
    #[serde(default)]
    pub quoted_source: Option<serde_json::Value>,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct PeerGroupMember {
    pub session_id: String,
    pub label: String,
    pub send: bool,
    pub receive: bool,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct PeerGroup {
    pub group_id: String,
    pub board_id: String,
    pub revision: i64,
    pub authorization_epoch: i64,
    pub enabled: bool,
    pub members: Vec<PeerGroupMember>,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct PeerMessage {
    pub message_id: String,
    #[serde(default)]
    pub from_session_id: Option<String>,
    pub to_session_id: String,
    #[serde(default)]
    pub group_id: Option<String>,
    #[serde(default)]
    pub chain_id: Option<String>,
    pub hop: i64,
    pub state: String,
    #[serde(default)]
    pub turn_id: Option<String>,
    #[serde(default)]
    pub display_message: Option<String>,
    pub origin: MessageOrigin,
    #[serde(default)]
    pub error: Option<String>,
    #[serde(default)]
    pub created_at: Option<f64>,
    #[serde(default)]
    pub updated_at: Option<f64>,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct QueuedPromptEntry {
    #[serde(default)]
    pub message_id: Option<String>,
    pub message: String,
    pub state: String,
    pub origin: MessageOrigin,
    #[serde(default)]
    pub error: Option<String>,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct BrowserTarget {
    pub target_id: String,
    pub url: String,
    pub title: String,
    pub active: bool,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct BrowserContext {
    pub session_id: String,
    pub supported: bool,
    pub host_registered: bool,
    pub context_state: String,
    #[serde(default)]
    pub state: Option<String>,
    #[serde(default)]
    pub available: Option<bool>,
    #[serde(default)]
    pub backend: Option<String>,
    #[serde(default)]
    pub context_id: Option<String>,
    #[serde(default)]
    pub control: Option<String>,
    #[serde(default)]
    pub control_revision: Option<i64>,
    #[serde(default)]
    pub active_target_id: Option<String>,
    #[serde(default)]
    pub targets: Option<Vec<BrowserTarget>>,
    #[serde(default)]
    pub control_state: Option<String>,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct BrowserControl {
    pub control: String,
    pub control_revision: i64,
    #[serde(default)]
    pub control_state: Option<String>,
    #[serde(default)]
    pub outstanding_mutations: Option<i64>,
}
